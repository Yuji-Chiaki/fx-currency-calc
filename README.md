...
| `/api/calculate-margin` | POST | 証拠金計算 | `currencyPair`, `lots`, `leverage`, `rate` |
...
- **外部API**: 将来的に為替レートAPIと連携予定

### データフロー
1. フロントエンドがページ読み込み時にAPIを呼び出し
2. バックエンド（Hono）がデータを処理・計算
3. 結果をJSON形式でレスポンス
4. フロントエンドが結果を整形して表示
5. 30秒ごとにレート自動更新
...
- このツールは**教育目的のデモアプリケーション**です
- 表示されているレートは**デモデータ**であり、実際の市場レートではありません
