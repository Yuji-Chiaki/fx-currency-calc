...
- **特徴**: 
  ...
  - **実際の為替レートを使用（Frankfurter API - 欧州中央銀行データ）** 🌟本番対応
  - レスポンシブデザインでモバイル対応
...
3. **リアルタイムレート表示（本番対応）** 🌟
   ...
   - **Frankfurter API使用（欧州中央銀行データ）** 🌟NEW
   - **完全無料・APIキー不要** 🌟NEW
...
### 🌟 Frankfurter API（欧州中央銀行データ）

このアプリケーションは**実際の市場レート**を使用しています。

**API情報:**
- **提供元**: Frankfurter API (https://www.frankfurter.app/)
- **データソース**: 欧州中央銀行（ECB）
- **更新頻度**: 営業日は毎日更新（通常16:00 CET頃）
- **料金**: 完全無料
- **APIキー**: 不要
- **レート制限**: なし
- **対応通貨**: 30通貨以上
...
- **外部API**: Frankfurter API（欧州中央銀行データ）🌟実装済み

### データフロー
1. フロントエンドがページ読み込み時にAPIを呼び出し
2. バックエンド（Hono）がFrankfurter APIから最新レートを取得 🌟
3. 取得したレートをJPY建てに変換して処理・計算 🌟
4. 結果をJSON形式でレスポンス
5. フロントエンドが結果を整形して表示
6. 1分ごとに自動的に最新レートを再取得 🌟
...
**主な更新内容:**
- 🌟 **実際の為替レートAPIを統合（Frankfurter API - ECB公式データ）**
- 🌟 **本番対応：デモデータから実際の市場レートに変更**
